/**
 * @file native_test_binlog_reader.h
 * @brief Native in-tree replacement for the external test_binlog_reader-t binary.
 * @details This helper reproduces the behavior of the external helper from
 *   the proxysql_binlog_test repository. It uses the MariaDB client replication
 *   API (MARIADB_RPL) to send COM_REGISTER_SLAVE natively by setting rpl->host
 *   before calling mariadb_rpl_open().
 *
 *   The helper runs exactly two replication sessions, matching the external
 *   helper's behavior. Each session:
 ...
 *     5. Opens replication and fetches events to prove the stream is active
 *     6. Closes the replication session cleanly
 *
 *   Returns EXIT_SUCCESS only if both sessions succeed.
 */

#ifndef NATIVE_TEST_BINLOG_READER_H
#define NATIVE_TEST_BINLOG_READER_H
...
#define NATIVE_BINLOG_READER_NHB 3
 ...
 *   variables, then uses MARIADB_RPL with rpl->host set to trigger
 *   COM_REGISTER_SLAVE. Fetches events until NATIVE_BINLOG_READER_NHB heartbeats are
 *   received, proving the binlog stream is active.
 *
...
static int run_single_replication_session(const CommandLine& cl, int session_id, int server_id) {
	...
	// Issue a lightweight pre-replication query, matching the external helper's
	// behavior of running a normal query before initiating replication.
	...
	diag("Session %d: Fetching binlog events, waiting for %d heartbeats...",
		session_id, NATIVE_BINLOG_READER_NHB);

	while (num_heartbeats < NATIVE_BINLOG_READER_NHB &&
		...
		if (event->event_type == HEARTBEAT_LOG_EVENT_V2 ||
			...
			diag("Session %d: Heartbeat %d/%d received (total events: %d)",
				session_id, num_heartbeats, NATIVE_BINLOG_READER_NHB, num_events);
		}
	}

	if (num_heartbeats < NATIVE_BINLOG_READER_NHB && event == NULL) {
	...
	if (num_heartbeats < NATIVE_BINLOG_READER_NHB) {
		diag("Session %d: FAILED - received only %d/%d heartbeats",
			session_id, num_heartbeats, NATIVE_BINLOG_READER_NHB);
		return EXIT_FAILURE;
	}
...
/**
 * @brief Native replacement for the external test_binlog_reader-t binary.
 * @details Runs exactly two replication sessions, each sending
 *   COM_REGISTER_SLAVE via MARIADB_RPL. This matches the external helper's
 *   effective behavior: two independent fast-forward replication sessions.
 *
 *   Both TAP tests that previously depended on the external binary can call
 *   this function directly:
 *     - test_com_register_slave_enables_fast_forward-t: checks that the
 *       function returns EXIT_SUCCESS (proving COM_REGISTER_SLAVE + binlog works)
 *     - test_binlog_reader_uses_previous_hostgroup-t: checks that exactly 2
 *       fast-forward connections were closed in the target hostgroup
 *
 * @param cl CommandLine with connection parameters.
 * @return EXIT_SUCCESS if both sessions succeed, EXIT_FAILURE otherwise.
 */
static int run_native_test_binlog_reader(const CommandLine& cl) {
	diag("=== Native test_binlog_reader: Starting ===");
	diag("Running two replication sessions to exercise COM_REGISTER_SLAVE");

	// Session 1
	int rc = run_single_replication_session(cl, 1, 100001);
	if (rc != EXIT_SUCCESS) {
		diag("=== Native test_binlog_reader: FAILED (session 1) ===");
		return EXIT_FAILURE;
	}
	...
	if (rc != EXIT_SUCCESS) {
		diag("=== Native test_binlog_reader: FAILED (session 2) ===");
		return EXIT_FAILURE;
	}

	diag("=== Native test_binlog_reader: SUCCESS (both sessions completed) ===");
	return EXIT_SUCCESS;
}

#endif // NATIVE_TEST_BINLOG_READER_H
