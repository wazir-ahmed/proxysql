...
int wait_for_replacement_probe(MYSQL* admin, RDS_BGD_Simulator& sim, uint64_t sequence,
	const string& scenario, RDS_BGD_Cluster& cluster, const BGD_Hostgroups& hgs, int green_ssl)
{
	auto [table_rc, table_probe] = bgd_wait_for_probe(sim, sequence, cluster.blue_writer.endpoint(),
		RDS_BGD_Probe_Kind::table_check, kProbeTimeoutMs, 0, admin, scenario, "replacement table check",
		hgs.blue_writer, { hgs.blue_writer, hgs.blue_reader, hgs.green_writer, hgs.green_reader });
	if (table_rc != EXIT_SUCCESS) return table_rc;
	auto [blue_metadata_rc, blue_metadata] = bgd_wait_for_probe(sim, table_probe.sequence_id,
	...
	if (blue_metadata_rc != EXIT_SUCCESS) return blue_metadata_rc;
	auto [metadata_rc, metadata_probe] = bgd_wait_for_probe(sim, blue_metadata.sequence_id,
...
int main() {
	...
	int initial_status_rc = setup_rc == EXIT_SUCCESS ? wait_for_status(admin, sim, initial_seq,
		"replacement-input", original_hgs, "initial in-progress", "WRITER_SWITCHOVER_IN_PROGRESS") : EXIT_FAILURE;
	int initial_effects_rc = initial_status_rc == EXIT_SUCCESS ? wait_for_writer_placement(admin, sim, initial_seq,
		"replacement-input", original_hgs, replacement, "initial in-progress effects", true) : EXIT_FAILURE;
	ok(rc == EXIT_SUCCESS && setup_rc == EXIT_SUCCESS && initial_status_rc == EXIT_SUCCESS && initial_effects_rc == EXIT_SUCCESS,
		"pre-completion worker publishes in-progress state and its blue-writer demotion");
	...
	if (rc == EXIT_SUCCESS) rc = execute_all(admin, {
		"UPDATE mysql_servers SET status='OFFLINE_SOFT' WHERE hostgroup_id=1343 AND hostname=" +
	...
	int replacement_probe_rc = rc == EXIT_SUCCESS ? wait_for_replacement_probe(admin, sim, replacement_seq,
		"replacement-input", replacement_fresh, replacement_hgs, 1) : EXIT_FAILURE;
	int replacement_status_rc = replacement_probe_rc == EXIT_SUCCESS ? wait_for_status(admin, sim, replacement_seq,
	...
	ok(rc == EXIT_SUCCESS && replacement_probe_rc == EXIT_SUCCESS && replacement_status_rc == EXIT_SUCCESS &&
		replacement_effects_rc == EXIT_SUCCESS,
		"replacement probes from a table-check baseline and applies current TLS and in-progress policy");
	...
	ok(runtime_definition_matches(admin, replacement_hgs, 1, 1, 150, 900) &&
		runtime_server_absent(admin, original_hgs.green_reader, replacement.green_readers[1]),
		"replacement state uses current hostgroups without acting on the removed green mapping");
	...
	ok(rc == EXIT_SUCCESS && tls_replacement_rc == EXIT_SUCCESS,
		"relevant TLS input change restarts probing and uses the replacement TLS value");
	...
	ok(rc == EXIT_SUCCESS && completed_probe_rc == EXIT_SUCCESS && completed_replacement_status_rc == EXIT_SUCCESS,
		"completed-phase replacement starts fresh probes and republishes the accepted completed behavior");
