...
static constexpr const char* kPayloadSubdir = "readonly_tests_payloads";
...
static std::vector<std::string> get_test_files(const std::string& path) {
...
static std::pair<int, std::string> make_internal_error(
...
// Mirrors json_server_comparator from cluster_simulator/lib/common_utils.cpp:
// sorts per-server JSON objects by (hostgroup_id, hostname, port) so that two
// unordered cluster states can be diffed visually in TAP diagnostics.
static bool server_state_less(const ordered_json& a, const ordered_json& b) {
	auto key = [](const ordered_json& j) {
...
// Mirrors serialize_verification_error from cluster_simulator/lib/common_utils.cpp
// (inlined so the wrapper avoids linking cluster_simulator_lib.a): stringify a
// single verification_error entry with its expected/actual state arrays sorted
// by (hostgroup_id, hostname, port) and indented for TAP diagnostic output.
static std::pair<int, std::string> serialize_verification_error(
	const ordered_json& j_err, std::string& out_error_str
) {
	try {
...
// Flattens the simulator's `results` array (each entry either a
// verification_error or something else) into a single human-readable string
// suitable for a TAP diagnostic.
static std::pair<int, std::string> serialize_errors(
	const ordered_json& j_results, std::string& out_str_errs
) {
