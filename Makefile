...
.PHONY: build_src_testawsrds
build_src_testawsrds: build_lib_testawsrds
	cd src && OPTZ="${O0} -ggdb -DDEBUG -DTEST_AWS_RDS" CC=${CC} CXX=${CXX} ${MAKE}
...
.PHONY: build_lib_testawsrds
build_lib_testawsrds: build_deps_debug
	cd lib && OPTZ="${O0} -ggdb -DDEBUG -DTEST_AWS_RDS" CC=${CC} CXX=${CXX} ${MAKE}
